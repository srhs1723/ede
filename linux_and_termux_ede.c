...
int module_load(const char *emod_path) {
    ...
    /* Load shared object with dlopen */
    mod->handle = dlopen(emod_path, RTLD_LAZY | RTLD_LOCAL);
    
    if (!mod->handle) {
        const char *error = dlerror();
        log_message("Failed to load module %s: %s", emod_path, error ? error : "unknown error");
        return -1;
    }
    ...
    const char* (*get_name)(void) = (const char*(*)(void))dlsym(mod->handle, "get_module_name");
    const char* (*get_version)(void) = (const char*(*)(void))dlsym(mod->handle, "get_module_version");
    const char* (*get_author)(void) = (const char*(*)(void))dlsym(mod->handle, "get_module_author");
    const char* (*get_description)(void) = (const char*(*)(void))dlsym(mod->handle, "get_module_description");
    ...
    mod->init = (int(*)(ModuleAPI*))dlsym(mod->handle, "module_init");
    mod->cleanup = (void(*)(void))dlsym(mod->handle, "module_cleanup");
    mod->on_key = (void(*)(int))dlsym(mod->handle, "module_on_key");
    mod->on_save = (void(*)(const char*))dlsym(mod->handle, "module_on_save");
    mod->on_load = (void(*)(const char*))dlsym(mod->handle, "module_on_load");
    mod->on_cursor_move = (void(*)(int, int))dlsym(mod->handle, "module_on_cursor_move");
    mod->on_mode_change = (void(*)(int, int))dlsym(mod->handle, "module_on_mode_change");
    mod->on_text_insert = (void(*)(const char*))dlsym(mod->handle, "module_on_text_insert");
    mod->on_text_delete = (void(*)(int, int))dlsym(mod->handle, "module_on_text_delete");
    mod->on_tab_switch = (void(*)(int, int))dlsym(mod->handle, "module_on_tab_switch");
    mod->execute_command = (int(*)(const char*, const char*))dlsym(mod->handle, "module_execute_command");
    mod->set_config = (void(*)(const char*, const char*))dlsym(mod->handle, "module_set_config");
    mod->get_config = (const char*(*)(const char*))dlsym(mod->handle, "module_get_config");
...
int esrc_compile(const char *esrc_path, const char *emod_path) {
    ...
    /* Compile C to shared object using system compiler */
...
void terminal_init(void) {
    ...
    if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) != -1) {
        g_state.screen_cols = ws.ws_col;
        g_state.screen_rows = ws.ws_row;
    } else {
        g_state.screen_cols = 80;
        g_state.screen_rows = 24;
    }
...
int terminal_get_key(void) {
    ...
    /* Map ncurses keys to editor keys */
    switch (ch) {
...
void editor_refresh_screen(void) {
    ...
    for (int i = 0; i < g_state.tab_count; i++) {
        if (i == g_state.active_tab) {
            mvprintw(row, 0, "[%d:%s*]", i + 1, g_state.tabs[i].name);
        } else {
            printw(" %d:%s ", i + 1, g_state.tabs[i].name);
        }
    }
    attroff(A_REVERSE);
    row++;
    ...
    if (tab->file_count > 0) {
        ...
        for (int i = 0; i < g_state.screen_rows - 3 && row < g_state.screen_rows - 2; i++, row++) {
            int line_len;
        ...
        /* Status bar */
        attron(A_REVERSE);
        mvprintw(g_state.screen_rows - 2, 0, "%-*s", g_state.screen_cols, g_state.status_message);
        attroff(A_REVERSE);
        
        /* Command line */
        if (g_state.mode == MODE_EXIT_CONFIRM) {
            mvprintw(g_state.screen_rows - 1, 0, "> %s", g_state.exit_command);
        } else {
            mvprintw(g_state.screen_rows - 1, 0, "Row:%d Col:%d %s %s", 
                   fb->cursor_row + 1, 
                   fb->cursor_col + 1,
                   fb->modified ? "[+]" : "",
